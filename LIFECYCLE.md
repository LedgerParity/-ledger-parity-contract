...
Protocol 28 testnet deployment, signed registration/read-back, an early renewal transaction and provider fee measurements are now recorded in [TESTNET.md](TESTNET.md). A [read-only monitor](tools/README.md) checks the public testnet registry and emits restoration keys. Actual instance/code and record restoration/read-back after expiry and an operator-owned monitoring process remain outstanding. Local Rust tests remain unavailable because the Windows MSVC linker is missing; Linux CI supplies native test evidence.
